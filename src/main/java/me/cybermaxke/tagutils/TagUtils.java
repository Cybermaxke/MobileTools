...
public class TagUtils {
	...
	public static ItemStack getItemStackFromTag(TagCompound tag) {
		...
		return CraftItemStack.asBukkitCopy(net.minecraft.server.v1_5_R2.ItemStack.createStack(c));
	}
	...
	public static TagCompound getInventoryAsTag(IInventory inventory) {
		TagCompound t = new TagCompound();
		for (int i = 0; i < inventory.getSize(); i++) {
			net.minecraft.server.v1_5_R2.ItemStack is = inventory.getItem(i);
			if (is != null) {
	...
	public static <T extends IInventory> T getInventoryFromTag(T inventory, TagCompound tag) {
		for (int i = 0; i < inventory.getSize(); i++) {
			if (tag.hasKey("Slot" + i)) {
				inventory.setItem(i, net.minecraft.server.v1_5_R2.ItemStack.createStack((NBTTagCompound)createTag(tag.getCompound("Slot" + i))));
			}
		}
		return inventory;
	}
